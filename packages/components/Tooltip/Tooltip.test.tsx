...
import { JoTooltip } from "."

import Tooltip from "./Tooltip.vue"

vi.mock("@popprtjs/core");

const onVisibleChange = vi.fn();

describe('Tooltip/index.ts', () => {
  // 测试WithInstall函数是否被正确应用
  it("should be exported with withInstall()", () => {
    expect(JoTooltip.install).toBeDefined();
  })

  // 测试Tooltip组件是否被正确导出
  it("should be export Tooltip component", () => {
    expect(JoTooltip).toBe(Tooltip);
  })

  // 可选：测试 withInstall 是否增强了 Tooltip 组件的功能
  test("should enhance Tooltip component", () => {
    const enhancedTooltip = withInstall(Tooltip);
    expect(enhancedTooltip).toBe(JoTooltip);
...
describe("Tooltip.vue", () => {
  ...
  test("basic tooltip", async () => {
    ...
    expect(wrapper.find(".jo-tooltip__popper").exists()).toBeFalsy();

    // 弹出层是否出现
    triggerArea.trigger("click");
    await vi.runAllTimers();
    expect(wrapper.find(".jo-tooltip__popper").exists()).toBeTruthy();
    expect(wrapper.get(".jo-tooltip__popper").text()).toBe("hello tooltip");
  ...
  test("tooltip with hover trigger", async () => {
    ...
    // 测试悬停显示
    wrapper.find(".jo-tooltip__trigger").trigger("mouseenter");
    await vi.runAllTimers();
    expect(wrapper.find(".jo-tooltip__popper").exists()).toBeTruthy();
    // 测试悬外隐藏
    wrapper.find(".jo-tooltip").trigger("mouseleave");
    await vi.runAllTimers();
    expect(wrapper.find(".jo-tooltip__popper").exists()).toBeFalsy();
  });
  ...
  test("tooltip with contextmenu trigger", async () => {
    ...
    // 测试右键菜单显示
    wrapper.find(".jo-tooltip__trigger").trigger("contextmenu");
    await vi.runAllTimers();
    expect(wrapper.find(".jo-tooltip__popper").exists()).toBeTruthy();
    // 测试右键菜单隐藏（可以模拟点击外部区域）
  });
  ...
  test("disabled tooltip", async () => {
    ...
    // 测试禁用状态下点击不会触发显示
    wrapper.find(".jo-tooltip__trigger").trigger("click");
    await vi.runAllTimers();
    expect(wrapper.find(".jo-tooltip__popper").exists()).toBeFalsy();
  });
  ...
  test("change trigger prop", async () => {
    ...
    wrapper.find(".jo-tooltip__trigger").trigger("hover");
