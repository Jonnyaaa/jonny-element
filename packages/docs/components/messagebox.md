...
调用 `JoMessageBox.alert` 方法以打开 alert 框。
...
调用 `JoMessageBox.confirm` 方法以打开 confirm 框。
...
调用 `JoMessageBox.prompt` 方法以打开 prompt 框。
...
可以通过配置 `JoMessageBox` 的参数来实现一些个性化需求（options 参数见下文）。
...
`JoMessageBox` 提供了全局方法 `JoMessageBox.alert`、`JoMessageBox.confirm`、`JoMessageBox.prompt`，用于在需要时弹出消息弹框。
...
```typescript
import { JoMessageBox } from "jonny-element";
```
