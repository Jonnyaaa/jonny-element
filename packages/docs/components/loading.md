...
```typescript
JoLoading.service(options);
```

LoadingService 会返回一个 Loading 实例，可通过调用该实例的 close 方法来关闭它

```typescript
const loading = JoLoading.service(options);
nextTick(() => {
  loading.close();
});
```
...
| jo-loading-text       | 加载文案         | `string`  |
| jo-loading-spinner    | 加载图标         | `string`  |
| jo-loading-background | 遮罩背景色       | `string`  |